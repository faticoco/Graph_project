   ...
   public:
    void path_finding() {
        while (nodes.get_size() > 0)  // while all nodes finish
        {
            ...
            while (counter < adj_nodes->get_size()) {
                ...
                // calculating path distance of the smallest adjacent node from
                // start
                int path_distance = calculate_distance(small, adj) +
                                    small->dist_from_the_first_node;
    ...
    NODE* get_smallest_node(linked_list<NODE*>& n) {
        ...
        while (counter < n.get_size()) {
            ...
            if (small->dist_from_the_first_node >
                node->dist_from_the_first_node) {
                small = node;
                s_location = counter;
            }
            counter++;
        }
        cout << "smallest node is " << small->id << endl;
		cout << "size of nodes is " << n.get_size() << endl;
		cout << "s_location is " << s_location << endl;
        for (int i = 0; i <= s_location; i++) {
            n.deletion(n.operator[](0));
        }
        return small;
    }

    linked_list<NODE*>* other_nodes(
        NODE* node)  // returns the other adjacent nodes
    {
    ...
    bool composition(linked_list<NODE*>& all_nodes,
                     NODE* node_contained_or_not) {
        int counter = 0;
    ...
    void distance_between_two_nodes(NODE* N1, NODE* N2) {
        ...
        path_finding();
        cout << "\nSocial circle between two people is of "
             << calculate_shortest_path(N2) << " people apart " << endl;
    }
};
#endif  // !path_graph_h